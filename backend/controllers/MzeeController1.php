...
class MzeeController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $msaidizi->load(Yii::$app->request->post())) {
            ...
            $msaidizid = Mzee::findOne(['nambar' => $_POST['MsaidiziMzee']['nambari_ya_kitambulisho'],'aina_ya_kitambulisho' => $_POST['MsaidiziMzee']['aina_ya_kitambulisho']]);
            $msaidizimzee = MsaidiziMzee::findOne(['aina_ya_kitambulisho'=> $_POST['MsaidiziMzee']['aina_ya_kitambulisho'],'nambari_ya_kitambulisho' => $_POST['MsaidiziMzee']['nambari_ya_kitambulisho'] ]);
            if($msaidizid != null || $msaidizimzee != null){
                Yii::$app->session->setFlash('', [
                    'type' => 'warning',
                    'duration' => 3000,
                    'icon' => 'fa fa-warning',
                    'message' => 'Namba ya kitambulisho cha msaidizi imeshatumika tayari',
                    'positonY' => 'top',
                    'positonX' => 'right'
                ]);
                return $this->render('create', [
            ...
            if ($model->picha != null) {
                ...
                if ($_POST['Mzee']['mzawa_zanzibar'] == 'N' && $_POST['Mzee']['tarehe_kuingia_zanzibar'] == " ") {
                    ...
                    if ($model->save()) {
                        ...
                        $msaidizi->picha = UploadedFile::getInstance($msaidizi, 'msaidizi_picha');
                        $msaidizi->mzee_id = $model->id;
                        $msaidizi->status = MsaidiziMzee::ACTIVE;
                        if($msaidizi->picha !=null) {
                            $msaidizi->picha->saveAs('uploads/wasaidizi/' . date('YmdHi') . '.' . $msaidizi->picha->extension);
                            $msaidizi->picha = date('YmdHi') . '.' . $msaidizi->picha->extension;
                            ...
                            if($msaidizi->save(false)) {
                                ...
                                Yii::$app->session->setFlash('', [
                                    ...
                                    'duration' => 1500,
                                    'icon' => 'fa fa-check',
                                    'message' => 'Usajili umekamilika,lakini mtu wa karibu hana picha',
                                    'positonY' => 'top',
                                    'positonX' => 'right'
                                ]);
                                return $this->redirect(['view', 'id' => $model->id]);
