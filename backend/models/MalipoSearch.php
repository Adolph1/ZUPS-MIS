    ...
    */
    ...
    public function search($params)
    {
        if(Yii::$app->user->can('Cashier') ) {
            ...
            return $dataProvider;
        }elseif(Yii::$app->user->can('DataClerk')){
            $query = Malipo::find();
            ...
            $subquery = Mzee::find()
                ->select('id')
                ->where(['wilaya_id' => Wafanyakazi::getDistrictID(Yii::$app->user->identity->user_id)]);
            $query->where(['status' => Malipo::PENDING]);
            $query->andWhere(['in', 'mzee_id', $subquery]);
            $query->orderBy(['tarehe_kulipwa' => SORT_DESC]);
    ...
    public function searchLeo($params)
    {
        if(Yii::$app->user->can('Cashier') ) {
            ...
            $subquery = Mzee::find()
                ->select('id')
                ->where(['wilaya_id' => Wafanyakazi::getDistrictID(Yii::$app->user->identity->user_id)]);
            $query->where(['in', 'mzee_id', $subquery]);
            $query->andWhere(['status' => Malipo::PAID]);
            $query->orderBy(['tarehe_kulipwa' => SORT_DESC]);
