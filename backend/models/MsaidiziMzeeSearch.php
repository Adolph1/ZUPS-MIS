...
class MsaidiziMzeeSearch extends MsaidiziMzee
{
    ...
    }    public function searchWithFingerAll($params)
    {
        $subquery = Mkoa::find()->select('id');
        $wazee = Mzee::find()->select('msaidizi_id')->where(['mzee_finger_print'=>null]);
        $query = MsaidiziMzee::find();
       $query->where(['in','id',$wazee]);
        $query->where(['!=','finger_print','']);
