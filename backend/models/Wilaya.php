...
class Wilaya extends \yii\db\ActiveRecord
{
    ...
    public static function getAll()
    {
        ...
        return ArrayHelper::map(Wilaya::find()->where(['in','mkoa_id',$mikoa])->all(),'id','jina');
    }
    ...
    public static function getAllByZone($getZoneByID)
    {
        $subquery = Mkoa::find()->select('id')->where(['zone_id'=>$getZoneByID]);
        $wilaya = Wilaya::find()->where(['in','mkoa_id',$subquery])->all();
        if($wilaya !=null){
            return $wilaya;
        }else{
            return null;
